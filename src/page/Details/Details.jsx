...
const productRatings = [
  { ctm: customer.name, id: 1, variation: "Ebony, size 2 meters", rating: 1, date: new Date("2022-02-04T23:59:00Z"), imgSrc: ImageReview, imgSrc1: ImageReview, imgSrc2: ImageReview, comment: "Terrible product" },
  { ctm: customer.name, id: 2, variation: "Ebony, size 2 meters", rating: 2, date: new Date("2022-02-04T23:59:00Z"), imgSrc: ImageReview, imgSrc1: ImageReview, imgSrc2: ImageReview, comment: "Not great, not terrible" },
  { ctm: customer.name, id: 3, variation: "Ebony, size 2 meters", rating: 3, date: new Date("2022-02-04T23:59:00Z"), imgSrc: ImageReview, imgSrc1: ImageReview, imgSrc2: ImageReview, comment: "Average product" },
  { ctm: customer.name, id: 4, variation: "Ebony, size 2 meters", rating: 4, date: new Date("2022-02-04T23:59:00Z"), imgSrc: ImageReview, imgSrc1: ImageReview, imgSrc2: ImageReview, comment: "Great product" },
  { ctm: customer.name, id: 5, variation: "Ebony, size 2 meters", rating: 5, date: new Date("2022-02-04T23:59:00Z"), imgSrc: ImageReview, imgSrc1: ImageReview, imgSrc2: ImageReview, comment: "Excellent product" },
];
...
const RatingBox = () => {
  ...
  return (
    <Grid>
      <Box border={1} borderColor='#1264A9' padding={3} display='flex' style={{ backgroundColor: '#EDFFFF', borderRadius: 10 }}>
        <Grid container justifyContent="left" alignItems="center">
          <Grid xs={5} >
            <Grid container display='flex' justifyContent='center' alignItems='baseline'>
              <Grid item>
                <Typography variant="h6" sx={{ color: '#F5B000', fontSize: '36px' }}>{averageRating}</Typography>
              </Grid>
              <Grid item>
                <Typography variant="h6" sx={{ color: '#F5B000', fontSize: '24px', lineHeight: '36px', verticalAlign: 'text-bottom' }}> / </Typography>
              </Grid>
              <Grid item>
                <Typography variant="h6" sx={{ color: '#F5B000', fontSize: '36px' }}>5</Typography>
              </Grid>
            </Grid>
          ...
          <Grid item xs={6} container direction="column" alignItems="center" justifyContent="center">
            <Grid container item justifyContent="center" alignItems="center" paddingBottom={0.5}>
              <Button
              ...
              {[1, 2, 3].map((rating) => (
                <Button
                  ...
                  sx={{ px: 1.5, mx: 1 }}
                >
                  {rating === 1 ? "1 star" : `${rating} stars`} ({ratingGroups[rating] || 0})
                </Button>
              ))}
            </Grid>
            <Grid container item justifyContent="center" alignItems="center" paddingTop={1.5}>
              {[4, 5].map((rating) => (
                <Button
                  key={rating}
                  variant={selectedRating === rating ? "contained" : "outlined"}
          ...
          {filteredRatings.map((rating) => (
            <Box key={rating.id} sx={{ my: 12 }}>
              <Box sx={{ display: 'flex', alignItems: 'flex-start' }}>
                <Avatar src={customer.avatar} alt={customer.name} sx={{ mr: 2, width: 64, height: 64 }} />
                <Box>
                  <Typography variant="subtitle2" sx={{ mb: 0.5, fontSize: '18px', fontWeight: 'bold' }}>{rating.ctm}</Typography>
                  <Typography variant="caption" sx={{ mb: 1, fontSize: '14px' }}>{`${rating.date.getFullYear()}-${(rating.date.getMonth() + 1).toString().padStart(2, '0')}-${rating.date.getDate().toString().padStart(2, '0')} ${rating.date.getHours().toString().padStart(2, '0')}:${rating.date.getMinutes().toString().padStart(2, '0')}`}</Typography>
                  <Typography variant="body1" sx={{ mb: 1, color: 'grey.500', fontSize: '18px', fontStyle: 'italic' }}>
                    <span style={{ fontWeight: 'bold' }}>Variation:</span> {rating.variation}
                  </Typography>
                  <Typography variant="body2" sx={{ mb: 2, fontSize: '18px' }}>{rating.comment}</Typography>
                  <Box sx={{ display: 'flex' }}>
                    <img src={rating.imgSrc} alt="Product review image" style={{ width: '120px', height: '120px', objectFit: 'contain', marginRight: '8px' }} />
                    <img src={rating.imgSrc1} alt="Product review image" style={{ width: '120px', height: '120px', objectFit: 'contain', marginRight: '8px' }} />
                    <img src={rating.imgSrc2} alt="Product review image" style={{ width: '120px', height: '120px', objectFit: 'contain' }} />
                  </Box>
                  <ThumbUpAltOutlinedIcon color="primary" sx={{ cursor: 'pointer' }} />
                </Box>
              </Box>
            </Box>
          ))}
